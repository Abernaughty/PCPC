...
module "static_web_app" {
  ...
  app_settings = {
    API_URL = "https://pcpc-apim-prod.azure-api.net/pcpc-api/v1"
    DEBUG   = "false"
  }
...
module "application_insights" {
  ...
  # Create production action groups for alerting
  action_groups = [
    {
      ...
      email_receivers = [
        ...
        {
          name          = "oncall"
          email_address = var.oncall_email_address
        }
      ]
      webhook_receivers = []
    }
  ...
  # Create production metric alerts with stricter thresholds
  metric_alerts = [
    {
      name        = "Critical Error Rate"
      description = "Triggers when error rate exceeds 2%"
      severity    = 0 # Critical
      frequency   = "PT1M"
      window_size = "PT5M"
      enabled     = true
    ...
    },
    {
      ...
      description = "Triggers when availability drops below 99.9%"
      severity    = 0 # Critical
      frequency   = "PT5M"
      window_size = "PT15M"
      enabled     = true
...
# API Management (Always enabled for production)
