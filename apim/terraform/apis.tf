...
# resource here. The OpenAPI spec import on azurerm_api_management_api.pcpc_api
# (which references apim/specs/pcpc-api-v1.yaml — see the /health entry there)
# creates and re-asserts this operation on every apply, so an explicit
# Terraform resource would race with the spec-imported one ("resource already
# exists" error on first apply). The other 3 operations (get_sets,
# get_cards_by_set, get_card_info) ARE explicit because they're referenced by
# `azurerm_api_management_api_operation_policy` resources for caching/backend
# policies; get_health has no operation-level policy, so the spec import alone
# is sufficient.
#
# Effect: the GET /health operation appears in APIM exactly once per apply,
# managed by the OpenAPI spec rather than by Terraform's resource graph.
