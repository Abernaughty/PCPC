...
import { monitoringService, pokeDataApiService } from "../../index";
import {
  CreditMonitoringData,
 ...
 */
export async function monitorCredits(
  myTimer: Timer,
  ...
  context.log(
    `${correlationId} Credit monitoring function executed at ${timestamp}`
  );
  ...
  try {
    ...
    context.log(
      `${correlationId} Step 1: Checking PokeData API credit balance...`
    );
    ...
    if (!creditStatus) {
      context.log(
        `${correlationId} ERROR: Unable to retrieve credit status (${creditCheckTime}ms)`
      );
    ...
    context.log(
      `${correlationId} Current credit status: ${creditsRemaining} credits remaining (${status}) - ${creditCheckTime}ms`
    );

    // Track credit check
    monitoringService.trackEvent("credits.checked", {
      creditsRemaining,
      status,
      correlationId,
    });
    monitoringService.trackMetric("credit.check.duration", creditCheckTime);
    monitoringService.trackMetric("credits.remaining", creditsRemaining);
    ...
    context.log(
      `${correlationId} Step 2: Initializing credit monitoring service...`
    );
    const creditMonitoringService = new CreditMonitoringService();

    // Step 3: Process credit status and generate monitoring data
    context.log(
      `${correlationId} Step 3: Processing credit status and generating alerts...`
    );
    ...
    context.log(
      `${correlationId} Credit status processed (${processingTime}ms) - Status: ${processedStatus.status}`
    );

    // Track processing
    monitoringService.trackMetric("credit.processing.duration", processingTime);
    monitoringService.trackEvent("credit.status.processed", {
    ...
    context.log(
      `${correlationId} Step 4: Creating comprehensive monitoring data structure...`
    );
    const monitoringData: CreditMonitoringData = {
      ...
      alerts:
        processedStatus.status !== "healthy"
          ? [
    ...
    if (processedStatus.usageSinceLastCheck) {
      ...
      context.log(
        `${correlationId} - Estimated daily usage: ${dailyUsageEstimate.toFixed(
          1
        )} credits/day`
      );
      context.log(
        `${correlationId} - Estimated days remaining: ${daysRemaining.toFixed(
          1
        )} days`
      );
      ...
      monitoringService.trackMetric(
        "credits.usage.since_6h",
        processedStatus.usageSinceLastCheck
      );
      monitoringService.trackMetric(
        "credits.usage.daily_estimate",
        dailyUsageEstimate
      );
      monitoringService.trackMetric("credits.days_remaining", daysRemaining);
      ...
      if (dailyUsageEstimate > 100) {
        // More than 100 credits per day indicates potential issue
        ...
        // Track anomaly
        monitoringService.trackEvent("anomaly.detected", {
          type: "high_usage",
    ...
    monitoringService.trackMetric("monitoring.save.duration", saveTime);
    monitoringService.trackEvent("monitoring.saved", {
      correlationId,
    });

    // Step 7: Generate summary and recommendations
    context.log(
      `${correlationId} Step 7: Generating final summary and recommendations...`
    );
    ...
    switch (processedStatus.status) {
      ...
      case "critical":
        context.log(
          `${correlationId} 🔴 STATUS: Critical - ${creditsRemaining} credits remaining (review functions)`
        );
        break;
      case "exhausted":
        context.log(
    ...
    if (
      ...
      context.log(
        `${correlationId} - Check for runaway functions or infinite loops`
      );
      context.log(
        `${correlationId} - Consider disabling non-essential functions temporarily`
      );
      context.log(
        `${correlationId} - Monitor GetCardInfo and GetCardsBySet usage patterns`
      );
    }

    context.log(`${correlationId} 📈 Next monitoring check in 6 hours`);
  } catch (error) {
    const errorTotalTime = Date.now() - startTime;
    const errorMessage = error instanceof Error ? error.message : String(error);

    context.error(
      `${correlationId} ERROR in credit monitoring after ${errorTotalTime}ms: ${errorMessage}`
    );

    // Track error with Application Insights
    monitoringService.trackException(error as Error, {
      functionName: "MonitorCredits",
      duration: errorTotalTime,
      correlationId,
    });
    ...
    monitoringService.trackMetric("function.duration", errorTotalTime);
    ...
    try {
      ...
      await creditMonitoringService.logCreditEvent({
        ...
        functionName: "MonitorCredits",
        correlationId,
        details: { error: errorMessage, duration: errorTotalTime },
      });
    } catch (logError) {
